...
drop table if exists index2020.stat_vk_street;
create table index2020.stat_vk_street as
...
/* Индексы */
alter table index2020.stat_vk_street add primary key(id_street);
create index on index2020.stat_vk_street(id_gis);
create index on index2020.stat_vk_street(id_street);
create index on index2020.stat_vk_street(name);
create index on index2020.stat_vk_street(street_photos);
create index on index2020.stat_vk_street using gist(geom);

/* Комментарии */
comment on table index2020.stat_vk_street is 'Статистика по количесво фото vk сделанных на каждой именованной улице';
comment on column index2020.stat_vk_street.id_gis is 'id_gis города';
comment on column index2020.stat_vk_street.id_street is 'Уникальный идентификатор улицы';
comment on column index2020.stat_vk_street.name is 'Название улицы';
comment on column index2020.stat_vk_street.street_photos is 'Всего фото сделано на этой улице (фото находящиеся в радиусе 100 м. от оси улицы, для которых эта ось ближайшая)';


/* 34-й индикатор. Расчёт самого индикатора */
drop table  if exists index2020.ind_i34;
create table index2020.ind_i34 as
	select
		b.id_gis::smallint,
		b.city,
		b.region,
...
/* Индексы */
alter table index2020.ind_i34 add primary key(id_gis);

/* Комментарии */
comment on table index2020.ind_i34 is '34-й индикатор. Количество центров притяжения для населения.';
comment on column index2020.ind_i34.id_gis is 'Уникальный идентификатор города';
comment on column index2020.ind_i34.city is 'Город';
comment on column index2020.ind_i34.region is 'Субъект РФ';
comment on column index2020.ind_i34.total_photos is 'Всего фото в городе (вне помещения на расстоянии не далее 100 м. от ближайшей улицы)';
comment on column index2020.ind_i34.total_streets is 'Всего улиц в городе (именованных по OpenStreetMap)';
comment on column index2020.ind_i34.total_popular_streets is 'Всего популярных улиц (на которых суммарно сделано ~ 75% фото от общего числа)';
comment on column index2020.ind_i34.popular_streets_ratio is 'Отношение числа популярных улиц к общему числу улиц в городе (именованных по OpenStreetMap)';


/* Проверки */
/* Сравнение с 2019 годом. */
drop table if exists index2020.comp_i34;
create table index2020.comp_i34 as
select 
	i1.id_gis::smallint,
	i1.city,
	i1.region,
...
from index2020.ind_i34 i1
left join index2019.ind_i34 i2 using(id_gis)
order by id_gis;
--alter table index2019.ind_i34 rename column streets_sum_75_perc_photos to total_popular_streets;
/* Комментарии */
comment on table index2020.comp_i34 is 'Сравнение с 2019 годом. 34-й индикатор. Количество центров притяжения для населения.';
comment on column index2020.comp_i34.id_gis is 'Уникальный идентификатор города';
comment on column index2020.comp_i34.city is 'Город';
comment on column index2020.comp_i34.region is 'Субъект РФ';
comment on column index2020.comp_i34.total_photos_2019 is 'Общее количество фото, сделанных снаружи не далее 100 м от оси улицы в 2019 г., ед.';
comment on column index2020.comp_i34.total_photos_2020 is 'Общее количество фото, сделанных снаружи не далее 100 м от оси улицы в 2020 г., ед.';
comment on column index2020.comp_i34.total_streets_2019 is 'Число улиц в городе на 2019 г., ед.';
comment on column index2020.comp_i34.total_streets_2020 is 'Число улиц в городе на 2019 г., ед.';
comment on column index2020.comp_i34.total_popular_streets_2019 is 'Число улиц на которых сделано ~ 75% всех фото в городе на 2019 г., ед.';
comment on column index2020.comp_i34.total_popular_streets_2020 is 'Число улиц на которых сделано ~ 75% всех фото в городе на 2020 г., ед.';
comment on column index2020.comp_i34.popular_streets_ratio_2019 is 'Количество центров притяжения для населения - отношение улиц на которых сделано ~ 75% всех фото к общему числу фото, сделанных в городе в 2019 г.';
comment on column index2020.comp_i34.popular_streets_ratio_2020 is 'Количество центров притяжения для населения - отношение улиц на которых сделано ~ 75% всех фото к общему числу фото, сделанных в городе в 2020 г.';
comment on column index2020.comp_i34.higher_value is 'В каком году показатель "Количество центров притяжения для населения" выше';
